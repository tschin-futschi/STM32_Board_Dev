...
| I2C2 | PB10(SCL)/PB11(SDA)，**软件 bit-banging**，1 MHz，电机驱动 IC（地址运行时设置）；**SCL 推挽输出**抗串扰，SDA 开漏慢速沿 |
...
>
> **抗干扰（重要）**：部分板子（如 8# 板）I2C 线路无保护，SCL/SDA 间耦合强，软件 I2C 易被干扰。因此 **SCL 必须配成推挽输出**（单主机时钟恒由主机驱动，低阻强驱动、边沿干净、对 SDA 串扰免疫）；SDA 仍为开漏（从机需驱动 SDA 做 ACK/读）并用慢速沿减小耦合。**I2C 的 GPIO 口用推挽输出才能健壮。**
...
| — | I2C2 抗串扰：SCL 改推挽（2026-06-04，8# 板现场问题） | ✅ |
...
### 2026-06-04 I2C2 抗串扰：SCL 改推挽

现场发现 **8# 板**扫描电机线（I2C2 软件 bit-bang）时 SCL 时钟出现毛刺、扫描解码乱码，而同板上别人的固件正常。逐步排查（引脚识别测试确认 PB10/PB11 接线无误、SDA 静止时 SCL 恢复干净）定位为：该板 I2C 线路**无保护**、SCL-SDA 间耦合强，原 **SCL 配开漏**（释放即高阻）被 SDA 跳变窜扰，时钟被破坏（1 MHz / 400 kHz 同样复现，与速率无关）。

修复（`BSP_I2C2_Init`）：**SCL 改推挽输出**（master 单向驱动时钟，低阻、抗耦合、边沿干净）+ **SDA 保持开漏并降为慢速沿**（软化数据边沿、减小窜入 SCL 的耦合）。I2C2 速率仍为 1 MHz（刚性需求）。结论：**软件 I2C 的时钟线（SCL）须用推挽输出才能健壮。**
