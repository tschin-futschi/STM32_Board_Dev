...
| UART | USART1, PA9/PA10, AF7 | `BSP_UART_` | APB2, 460800 默认 |
...
**寄存器地址宽度**：电机 IC 寄存器地址为 **16-bit**，I2C 写操作时先发高字节再发低字节（大端）。BSP 层 `reg`/`startReg` 参数类型均为 `uint16_t`，App 层同理。**禁止**将寄存器地址强制转换为 `uint8_t`。
...
| 3 | I2C 驱动（I2C1/2/3） + Scan | I2C 扫描正常运行 | **完成** |
| 3.2 | 协议格式确定（帧格式 + 命令字，v1.4） | 开发者确认协议文档 | **完成** |
| 3.4 | 串口指令解析基础框架 | 能解析控制帧并响应心跳 | **完成** |
| 3.5 | PMIC 配置（RT5112WSC，I2C3） | PMIC 输出正常，电机 IC 供电安全 | **完成** |
| 4.1 | 采样定时器 BSP（`bsp_tim.c`） | TIM6 中断可触发，编译通过 | **完成** |
...
0. **语言约束（强制）**：所有输出（对话、注释、文档）只允许使用**汉语**或**英语**，禁止出现任何其他语言
1. **路线图查询必须核查代码（强制）**：每次开发者询问路线图或项目进度时，必须先检查相关模块的实际代码实现情况，不得仅依赖记忆或 TRACKING/ 中的历史记录。核查完成后列出当前真实状态，并询问开发者是否需要更新路线图
2. **先计划，后执行（强制）**：每次接到任务，必须先读取相关文件、列出修改计划并等待开发者确认，确认后才可动手修改。禁止跳过计划直接写代码
