...
| 4.1 | 采样定时器 BSP（`bsp_tim.c`） | TIM6 中断可触发，编译通过 | 未开始 |
| 4.2 | 系统控制命令（`0x02~0x05`） | 上位机发指令，STM32 正确响应 | **进行中** |
| 4.3 | 寄存器读写命令（`0x20~0x22`，新建 `app_motor.c`） | 读写 PMIC/INA 寄存器验证 | 未开始 |
| 4.4 | 采样控制命令（`0x50~0x54`，新建 `app_sample.c`） | 上位机能看到持续 `0xBB` 数据流 | 未开始 |
