...
/**
  * @brief Configure PB6/PB7 as AF open-drain for I2C1.
  */
static void ConfigGpioAF(void)
{
    GPIO_InitTypeDef gpioInit;

    gpioInit.GPIO_Pin   = BSP_I2C1_SCL_PIN | BSP_I2C1_SDA_PIN;
    gpioInit.GPIO_Mode  = GPIO_Mode_AF;
    gpioInit.GPIO_Speed = GPIO_Speed_50MHz;
    gpioInit.GPIO_OType = GPIO_OType_OD;
    gpioInit.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_Init(BSP_I2C1_SCL_GPIO_PORT, &gpioInit);

    GPIO_PinAFConfig(BSP_I2C1_SCL_GPIO_PORT, BSP_I2C1_SCL_PIN_SOURCE, BSP_I2C1_SCL_AF);
    GPIO_PinAFConfig(BSP_I2C1_SDA_GPIO_PORT, BSP_I2C1_SDA_PIN_SOURCE, BSP_I2C1_SDA_AF);
}

/**
  * @brief Initialize I2C1 peripheral registers (400 kHz, 7-bit).
  */
static void I2C1PeriphInit(void)
{
    ...
    i2cInit.I2C_ClockSpeed          = BSP_I2C1_SPEED;

    I2C_Init(BSP_I2C1_PERIPH, &i2cInit);
    I2C_Cmd(BSP_I2C1_PERIPH, ENABLE);
}
...
/**
  * @brief  Initialize I2C1: clocks, GPIO, peripheral.
...
ErrorStatus BSP_I2C1_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(BSP_I2C1_SCL_GPIO_CLK, ENABLE);
    RCC_APB1PeriphClockCmd(BSP_I2C1_RCC_APB1,     ENABLE);
    ...
    if (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_BUSY) != RESET)
    {
        BSP_I2C1_RecoverBus();
...
/**
  ...
  *
  *         Procedure:
  *           1. Disable and software-reset I2C1
  *           2. Reconfigure PB6/PB7 as GPIO output open-drain
  *           3. Toggle SCL 9 times to let slave finish its byte
  *           4. Issue a manual STOP condition (SCL high, SDA low→high)
  *           5. Restore GPIO as AF and re-initialize I2C1
  */
void BSP_I2C1_RecoverBus(void)
{
    ...
    /* 1. Disable and software-reset I2C1 */
    I2C_Cmd(BSP_I2C1_PERIPH, DISABLE);
    I2C_SoftwareResetCmd(BSP_I2C1_PERIPH, ENABLE);
    I2C_SoftwareResetCmd(BSP_I2C1_PERIPH, DISABLE);
    I2C_DeInit(BSP_I2C1_PERIPH);

    /* 2. Reconfigure PB6/PB7 as GPIO output open-drain, drive both high */
    gpioInit.GPIO_Pin   = BSP_I2C1_SCL_PIN | BSP_I2C1_SDA_PIN;
    ...
    gpioInit.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_Init(BSP_I2C1_SCL_GPIO_PORT, &gpioInit);
    GPIO_SetBits(BSP_I2C1_SCL_GPIO_PORT, BSP_I2C1_SCL_PIN | BSP_I2C1_SDA_PIN);

    /* 3. Toggle SCL 9 times (~1.25 us half-period at 400 kHz) */
    for (i = 0U; i < 9U; i++)
    {
        GPIO_ResetBits(BSP_I2C1_SCL_GPIO_PORT, BSP_I2C1_SCL_PIN);
        for (delay = 0U; delay < 200U; delay++) { __NOP(); }
        GPIO_SetBits(BSP_I2C1_SCL_GPIO_PORT, BSP_I2C1_SCL_PIN);
        for (delay = 0U; delay < 200U; delay++) { __NOP(); }
    }

    /* 4. Manual STOP: SDA low while SCL high, then SDA high */
    GPIO_ResetBits(BSP_I2C1_SDA_GPIO_PORT, BSP_I2C1_SDA_PIN);
    for (delay = 0U; delay < 200U; delay++) { __NOP(); }
    GPIO_SetBits(BSP_I2C1_SDA_GPIO_PORT, BSP_I2C1_SDA_PIN);
    for (delay = 0U; delay < 200U; delay++) { __NOP(); }
...
static ErrorStatus WritePhase(uint8_t devAddr, uint8_t reg)
{
    ...
    while (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_BUSY) != RESET)
    {
        if (--timeout == 0U) { return ERROR; }
    }

    /* START */
    I2C_GenerateSTART(BSP_I2C1_PERIPH, ENABLE);
    timeout = BSP_I2C1_TIMEOUT;
    while (!I2C_CheckEvent(BSP_I2C1_PERIPH, I2C_EVENT_MASTER_MODE_SELECT))
    {
        if (--timeout == 0U) { return ERROR; }
    }

    /* Device address + WRITE */
    I2C_Send7bitAddress(BSP_I2C1_PERIPH, (uint8_t)(devAddr << 1U),
                        I2C_Direction_Transmitter);
    timeout = BSP_I2C1_TIMEOUT;
    while (!I2C_CheckEvent(BSP_I2C1_PERIPH,
                           I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
    {
        /* AF set means NACK — device not present, fail immediately */
        if (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_AF) != RESET)
        {
            I2C_ClearFlag(BSP_I2C1_PERIPH, I2C_FLAG_AF);
            return ERROR;
        }
        if (--timeout == 0U) { return ERROR; }
    }
    ...
    /* Register address */
    I2C_SendData(BSP_I2C1_PERIPH, reg);
    timeout = BSP_I2C1_TIMEOUT;
    while (!I2C_CheckEvent(BSP_I2C1_PERIPH, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
    {
        if (--timeout == 0U) { return ERROR; }
    }
                               ...
                               const uint8_t *pData, uint16_t len)
{
    ...
    for (i = 0U; i < len; i++)
    {
        I2C_SendData(BSP_I2C1_PERIPH, pData[i]);
        timeout = BSP_I2C1_TIMEOUT;
        while (!I2C_CheckEvent(BSP_I2C1_PERIPH, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
        {
    ...
    I2C_GenerateSTOP(BSP_I2C1_PERIPH, ENABLE);
    return SUCCESS;
                              ...
                              uint8_t *pData, uint16_t len)
{
    ...
    if (len == 1U)
    {
        ...
        timeout = BSP_I2C1_TIMEOUT;
        while (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_ADDR) == RESET)
        {
            if (--timeout == 0U) { goto error; }
        }
        I2C_AcknowledgeConfig(BSP_I2C1_PERIPH, DISABLE);
        /* Clear ADDR by reading SR1 then SR2 */
        (void)BSP_I2C1_PERIPH->SR1;
        (void)BSP_I2C1_PERIPH->SR2;
        I2C_GenerateSTOP(BSP_I2C1_PERIPH, ENABLE);

        timeout = BSP_I2C1_TIMEOUT;
        while (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_RXNE) == RESET)
        {
            if (--timeout == 0U) { goto error; }
        }
        pData[0] = I2C_ReceiveData(BSP_I2C1_PERIPH);
    }
    else
    {
        ...
        I2C_AcknowledgeConfig(BSP_I2C1_PERIPH, ENABLE);
        ...
        for (i = 0U; i < len; i++)
        {
            ...
            pData[i] = I2C_ReceiveData(BSP_I2C1_PERIPH);
        }
    }
...
/**
  ...
  *         Identical to BSP_I2C1_ReadReg — provided for API symmetry.
  */
ErrorStatus BSP_I2C1_ReadRegs(uint8_t devAddr, uint8_t startReg,
                               uint8_t *pData, uint16_t len)
{
    return BSP_I2C1_ReadReg(devAddr, startReg, pData, len);
}
...
ErrorStatus BSP_I2C1_Scan(uint8_t *pAddrList, uint8_t *pCount)
{
    ...
    *pCount = 0U;
    ...
    for (addr = 1U; addr <= 127U; addr++)
    {
        ...
        while (I2C_GetFlagStatus(BSP_I2C1_PERIPH, I2C_FLAG_BUSY) != RESET)
        {
            if (--timeout == 0U) { BSP_I2C1_RecoverBus(); break; }
        }
        ...
        I2C_Send7bitAddress(BSP_I2C1_PERIPH, (uint8_t)(addr << 1U),
                            I2C_Direction_Transmitter);
        timeout = BSP_I2C1_TIMEOUT;
        while (!I2C_CheckEvent(BSP_I2C1_PERIPH,
                               I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
